...
class BaseTapListProvider:
    ...
    def get_provider(cls, provider_name):
        ...
        subclasses = {i.provider_name: i for i in cls.__subclasses__()}
        try:
    ...
    def get_venues(self):
        if not self.provider_name:
            raise ValueError("You must define `provider_name` in your __init__()")
        LOG.debug("Looking for venues with tap list provider %s", self.provider_name)
        queryset = (
    ...
    def get_manufacturer(self, name: str, **defaults) -> Manufacturer:
        ...
        for field in unique_fields:
            ...
            if not manufacturer:
                options = list(
                    Manufacturer.objects.filter(Q(**{field: value}) | Q(name=name))
                )
                if len(options) > 1:
                    LOG.info(
                    ...
                    manufacturer = [i for i in options if getattr(i, field)][0]
                elif options:
        ...
        if not manufacturer:
            ...
            try:
                manufacturer = Manufacturer.objects.filter(filter_expr).distinct().get()
            except Manufacturer.DoesNotExist:
                manufacturer = Manufacturer.objects.create(name=name, **kwargs)
...
def fix_urls(defaults):
    ...
    for field, expected_domain in domain_map.items():
        ...
        if expected_domain not in given_domain:
            ...
            try:
                real_field = [
                    key for key, val in domain_map.items() if val in given_domain
                ][0]
            except IndexError:
                LOG.warning(
