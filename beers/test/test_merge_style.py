...
class StyleMergeTestCase(TestCase):
    def test_merge_success(self):
        ...
        self.assertEqual(
            kept_style.alternate_names.count(),
            9,
        )
        self.assertEqual(
    ...
    def test_merge_duplicate_names(self):
        styles = Style.objects.bulk_create(StyleFactory.build() for dummy in range(3))
        StyleAlternateName.objects.bulk_create(
            StyleAlternateName(name=i.name, style=styles[0]) for i in styles[1:]
        )
