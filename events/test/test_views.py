...
class TestEventListTestCase(APITestCase):
    ...
    def test_post_request_with_valid_data_succeeds(self):
        response = self.client.post(
            self.url, json.dumps(self.event_data), content_type="application/json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)

        event = Event.objects.get(pk=response.data.get("id"))
        self.assertEqual(event.title, self.event_data.get("title"))
        self.assertEqual(event.description, self.event_data.get("description"))
...
class TestEventDetailTestCase(APITestCase):
    ...
    def test_patch_request_updates_a_venue(self):
        ...
        self.assertEqual(event.title, new_name)
